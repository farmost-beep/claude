...
> 整合自：Harness深度解析 / Anthropic Engineering Blog / SDK类型声明 / 运行时配置分析 / VILA-Lab论文 / 社区逆向工程
>
> 版本 v1.0 | 2026年6月3日 | 用于理解 Claude Code (v2.1.161) 内部工作原理
...
Claude Code 约 **51.2 万行 TypeScript**（基于 Bun 运行时），其中仅约 **1.6% 是 AI 决策逻辑**，其余 **98.4% 是确定性基础设施**（权限门控、上下文管理、工具路由、状态恢复）[VILA-Lab, arXiv:2604.14228]。
...
| 层级 | 组件 | 职责 |
|:----|:----|:----|
| **记忆层** | CLAUDE.md / MEMORY.md / Auto Memory | 六层记忆体系，跨会话知识持久化 |
| **扩展层** | Skills / SubAgents / Hooks / MCP | 可热插拔的行为与工具扩展 |
| **集成层** | MCP 服务器 / LSP / OS Shell | 连接外部世界 |
| **编程层** | Agent SDK / Workflow Engine | 可编程的 Agent 框架 |

### 1.3 五层子系统（51.2 万行 TypeScript）

| 层级 | 名称 | 职责 | 关键文件 |
|:---:|:----|:----|:--------|
| 1 | **Surface**（表层） | CLI、Headless、SDK、IDE 前端 | React + Ink 终端 UI |
| 2 | **Core**（核心） | queryLoop、5阶段压缩管线、子代理调度 | QueryEngine.ts (约46,000行) |
| 3 | **Safety/Action**（安全/行动） | 7种权限模式、Auto Mode ML分类器、27 Hook事件、工具池、Shell沙箱 | bashSecurity.ts (23项安全检查) |
| 4 | **State**（状态） | Append-only JSONL transcripts、CLAUDE.md层级、Auto Memory、Sidechain | 追加写入日志系统 |
| 5 | **Backend**（后端） | Shell执行、MCP连接（8种传输）、42个工具子目录 | MCP连接管理器 |
...
| 模式 | 行为 | 适用场景 |
|:---:|:----|:--------|
| `plan` | 只读探索，禁止所有写入和Shell操作 | 只看不动 |
| `default` | 标准交互式审批 | 日常使用 |
| `acceptEdits` | 文件编辑自动批准，Shell操作仍需确认 | 开发者日常 |
| `dontAsk` | 白名单内全部操作自动批准 | 信任后提速 |
| `bypassPermissions` | 跳过所有检查 | 仅托管组织使用 |
| *(额外)* `auto` | ML分类器评估安全（2026新增） | 高频使用 |
| *(额外)* `bubble` | 子代理向父级升级权限 | Agent间通信 |

### 4.3 Auto Mode 双层 ML 分类器

```
Stage 1: 输入层——Prompt注入探针
  → 在工具输出进入Agent上下文前检测可疑内容
  → 单token yes/no，假阳性率 8.5%，假阴性率 6.6%

Stage 2: 输出层——Transcript分类器（基于Sonnet 4.6）
  → 仅Stage 1标记可疑时触发
  → Chain-of-thought推理，假阳性率降至 0.4%，假阴性率升至 17%
```

17% 的假阴性率意味着：在真实过度积极行为数据集中，17% 的危险操作未被阻断——这是已知的架构性局限。

### 4.4 七层独立安全防御

一个请求必须通过**所有适用层**，任何一层都可以阻断：
...
### 4.8 Hook 事件系统（27 个事件）

基于用户配置和已有文档分析，Hook 事件大致分为以下类别（完整清单需逆向分析二进制）：

| 事件类别 | 事件示例 |
|:--------|:--------|
| **PreToolUse** | 每次工具调用前触发，可阻断执行 |
| **PostToolUse** | 工具执行后触发，可检查结果 |
| **PreQuery** | LLM 推理前触发 |
| **PostQuery** | LLM 推理后触发 |
| **SessionStart** | 会话启动时触发 |
| **SessionEnd** | 会话结束时触发 |
| **ContextCompress** | 上下文压缩触发时 |
| **SubAgentStart** | 子Agent启动时触发 |
| **SubAgentEnd** | 子Agent结束时触发 |
| **Error** | 错误发生时触发 |

你项目的 `settings.json` 中已注册的 **4 类自动模式规则**（`python3 scripts/publish_articles.py *` 等）就是通过 Hook 系统实现的权限预批准。
