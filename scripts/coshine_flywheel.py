#!/usr/bin/env python3
"""
coshine_flywheel.py — Coshine（开先）AI启动飞轮

每周扫描4个维度：战略清晰度 → 执行进度 → 竞争适配 → 方法示范
对齐陈颖芳AI方法论4层框架，展示方法论在支付处理器公司的应用。

用法:
  python3 coshine_flywheel.py           # 扫描+微信推送
  python3 coshine_flywheel.py --dry-run # 只打印报告
"""
...
CASE_DIR = DELIVERABLES / "ai/AI方法论/案例/coshine"
...
def assess_战略清晰度() -> dict:
    ...
    strategy = CASE_DIR / "coshine_ai_strategy.md"
    if not strategy.exists():
        evidence["gaps"].append("战略文档缺失——coshine_ai_strategy.md不存在")
        return evidence

    mtime = datetime.fromtimestamp(strategy.stat().st_mtime)
    days = (datetime.now() - mtime).days
    content = strategy.read_text(encoding="utf-8")

    # 检查14部门覆盖
    departments = ["管理层", "销售", "品牌", "项目", "BA", "研发", "测试",
                   "运营", "客服", "财务", "合规", "HR", "终极变化", "实施路线"]
    found = [d for d in departments if d in content]
    coverage = len(found) / len(departments)
    evidence["signals"].append(f"14部门覆盖：{len(found)}/14 ({coverage:.0%})")
...
def assess_执行进度() -> dict:
    ...
    progress = CASE_DIR / "coshine_ai_progress.md"
    if not progress.exists():
        evidence["gaps"].append("进度追踪文件不存在——coshine_ai_progress.md缺失")
        return evidence
    ...
    # 统计各Phase的checkbox状态（排除文件头部的图例行）
    ...
    # 检查Phase Gate
    gates = CASE_DIR / "coshine_phase_gates.md"
    if gates.exists():
...
def assess_竞争适配() -> dict:
    ...
    comp_scan = CASE_DIR / "coshine_competitive_scan.md"
    ...
    # 检查竞争对手覆盖
    competitors = ["FIS", "Fiserv", "Global Payments", "ACI Worldwide",
                   "Worldline", "Nexi", "Network International"]
    ...
    if "行业趋势信号" in content:
        ...
        if "待补充" in trend_section and len(trend_section.strip().split("\n")) <= 3:
            evidence["gaps"].append("行业趋势信号为空——未追踪支付处理器行业AI动态")
    else:
...
def assess_方法示范() -> dict:
    ...
    mapping = CASE_DIR / "coshine_methodology_mapping.md"
...
def 联动判断(战略: dict, 进度: dict, 竞争: dict, 示范: dict) -> tuple:
    ...
    if "模糊" in 战略_lvl:
        return ("🔴 方向需校准", "战略文档不够清晰", [
            "优先完善 coshine_ai_strategy.md — 确保14部门全部有输入→输出描述",
            "定义每个Phase的ROI量化目标（不仅是定性描述）",
            "确认三阶段路线的优先级逻辑是否与业务现状一致",
        ])

    if "滞后" in 进度_lvl:
        return ("⚠️ 执行滞后", "推进速度不匹配战略清晰度", [
            "将Phase 1的4个核心项（售前/项目/开发/知识库）从'规划中'推进到'已启动'",
            "为Phase 1的每个方向指定owner和第一个里程碑日期",
            "检查是否有阻塞因素（资源/技术/决策）未在进度文件中标注",
        ])

    if "数据不足" in 竞争_lvl or "落后" in 竞争_lvl:
        return ("⚠️ 竞争预警", "竞争情报缺失，战略方向可能脱节", [
            "本周至少补充3家竞争对手的AI动态（FIS/Fiserv/ACI）",
            "搜索'payment processor AI 2026'判断行业AI趋势",
            "对比Coshine的Phase 1方向与竞对AI方向是否一致",
        ])

    if "弱" in 示范_lvl:
        return ("📝 案例待强化", "方法论展示不充分", [
            "补全未映射的原则到方法论映射文件中",
            "将弱映射原则升级到至少'中映射'（添加具体Coshine场景和可操作步骤）",
            "在案例README中说明'这个案例展示了方法论的哪些部分'",
        ])

    if len(real_gaps) == 0:
        return ("✅ 飞轮正常运转", "四维无实质性缺口，Coshine AI飞轮健康", [
            "维护当前节奏：每周进度更新 + 月度竞争扫描",
            "关注Phase Gate条件——准备Phase 1→2的过渡",
            "考虑将Coshine案例展示给相关方（方法论可复制性验证）",
        ])
